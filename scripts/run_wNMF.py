...
parser.add_argument('-g', '--keep_germline', help="Optional argument to keep germline variants. They are identified as annotated with True unter 'adata.var.dbSNP'." , action=argparse.BooleanOptionalAction)
...
parser.add_argument('-c', '--min_cell_cov', required=False, default=10, type=int, help="Optional argument defining minimum coverage in variant position of a cell for it to be kept in the anndata object. (Default of 10.)")
...
path = args.dir
keep_germline, min_MAF, min_cell_cov = args.keep_germline, args.min_MAF, args.min_cell_cov
k, max_cycles = args.n_factors, args.n_cycles

filter_germline=True if keep_germline is None else False
...
bootstrap_wNMF(adata, k=k, max_cycles=max_cycles, # n_bootstrap=5 ## for quick debugging
              )

print("Saving output.")
filename = path + "/out_adata"
